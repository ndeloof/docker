...
func (s *DockerSuite) TestPostContainersAttach(t *testing.T) {
	...
	expectSuccess := func(wc io.WriteCloser, br *bufio.Reader, stream string, tty bool) {
		...
		if !tty {
			...
			assert.Equal(t, actual[0], fdMap[stream])
		}
		assert.Assert(t, is.DeepEqual(actual[lenHeader:], expected), "Attach didn't return the expected data from %s", stream)
	}
	...
	expectTimeout := func(wc io.WriteCloser, br *bufio.Reader, stream string) {
		...
		actual := make([]byte, 1)
		_, err = readTimeout(br, actual, time.Second)
		assert.Assert(t, err.Error() == "Timeout", "Read from %s is expected to timeout", stream)
	}

	// Create a container that only emits stdout.
	cid, _ := dockerCmd(t, "run", "-di", "busybox", "cat")
	...
	cid, _ = dockerCmd(t, "run", "-di", "busybox", "/bin/sh", "-t", "cat >&2")
	cid = strings.TrimSpace(cid)
	wc, br, err = requestHijack(http.MethodPost, "/containers/"+cid+"/attach?stream=1&stdin=1&stderr=1", nil, "text/plain", request.DaemonHost())
	assert.NilError(t, err)
	...
	// Test with tty.
	cid, _ = dockerCmd(t, "run", "-dit", "busybox", "/bin/sh", "-t", "cat >&2")
	cid = strings.TrimSpace(cid)
	...
	cid, _ = dockerCmd(t, "run", "-di", "busybox", "/bin/sh", "-t", "echo hello; cat")
	cid = strings.TrimSpace(cid)
	...
	assert.NilError(t, err)
	assert.Equal(t, resp.ContentType, api.MediaTypeMultiplexedStream)
	...
	assert.Equal(t, outBuf.String(), "hello\nsuccess")
}
